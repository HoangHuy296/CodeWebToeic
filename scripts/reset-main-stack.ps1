...
try {
  ...
  $downExitCode = Invoke-DockerCompose -Arguments ($composeArgs + @("down", "-v", "--remove-orphans"))
  if ($downExitCode -ne 0) {
    throw "Failed to tear down the existing Docker stack."
  }

  Write-Host "Starting frontend + backend + mysql + nginx on a clean database..." -ForegroundColor Cyan
