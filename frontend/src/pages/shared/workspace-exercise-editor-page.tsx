...
import type { ExerciseTopic } from '../../types/exercise';
import type { MockTest, MockTestPayload, MockTestQuestion } from '../../types/mock-test';

interface WorkspaceExerciseEditorPageProps {
...
function createExerciseDraft(): MockTestPayload {
  return {
...
function mapExerciseToDraft(mockTest: MockTest): MockTestPayload {
  return {
    catalogKind: 'exercise',
    ...
    exerciseTopicSlug: mockTest.exerciseTopicSlug,
    questions: (mockTest.questions ?? []).map(mapQuestion),
  };
}

export function WorkspaceExerciseEditorPage({ workspaceRole, mode }: WorkspaceExerciseEditorPageProps) {
  const queryClient = useQueryClient();
  const navigate = useNavigate();
  const { slug } = useParams();
  const [draft, setDraft] = useState<MockTestPayload>(createExerciseDraft());
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);

  const detailQuery = useQuery({
    queryKey: [workspaceRole, 'exercise', 'detail', slug],
    queryFn: () => exerciseSessionApi.detail(slug!),
    enabled: mode === 'edit' && Boolean(slug),
  });
  const courseOptionsQuery = useQuery({
    queryKey: [workspaceRole, 'exercise-course-options'],
    queryFn: courseApi.manageMine,
  });
  const exerciseTopicsQuery = useQuery({
    queryKey: ['exercise-topics', 'workspace-options'],
  ...
  useEffect(() => {
    ...
    if (detailQuery.data) {
      setDraft(mapExerciseToDraft(detailQuery.data));
      setCurrentQuestionIndex(0);
    }
  }, [detailQuery.data, mode]);

  const saveMutation = useMutation({
    mutationFn: () => (mode === 'edit' && slug ? exerciseSessionApi.update(slug, draft) : exerciseSessionApi.create(draft)),
    onSuccess: async (result) => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: [workspaceRole, 'exercise', 'manage'] }),
        queryClient.invalidateQueries({ queryKey: ['admin', 'exercise-topics'] }),
        queryClient.invalidateQueries({ queryKey: ['mock-tests'] }),
        queryClient.invalidateQueries({ queryKey: ['public', 'exercise-topics'] }),
        queryClient.invalidateQueries({ queryKey: ['admin', 'stats'] }),
      ]);

      navigate(`/${workspaceRole}/exercises/items/${result.id}`, { replace: true });
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (exerciseId: string) => exerciseSessionApi.remove(exerciseId),
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: [workspaceRole, 'exercise', 'manage'] }),
        queryClient.invalidateQueries({ queryKey: ['admin', 'exercise-topics'] }),
      ...
      navigate(`/${workspaceRole}/exercises/items`, { replace: true });
    },
  });

  const availableCourses = courseOptionsQuery.data ?? [];
  const availableTopics = exerciseTopicsQuery.data ?? [];
  ...
  const selectedTopic = availableTopics.find((topic) => topic.slug === draft.exerciseTopicSlug);
  const canSubmit =
    Boolean(draft.title.trim()) &&
    Boolean(draft.description.trim()) &&
    Boolean(draft.exerciseTopicSlug) &&
  ...
  const pageTitle =
    workspaceRole === 'admin'
      ? mode === 'create'
        ? 'Admin tao bai tap moi de mo rong kho chu de on tap.'
        : 'Admin cap nhat bai tap on tap trong editor rieng.'
      : mode === 'create'
        ? 'Teacher tao bai tap ca nhan hoa cho hoc vien cua minh.'
        : 'Teacher cap nhat bai tap ca nhan hoa cho hoc vien cua minh.';

  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(28,25,23,0.94),rgba(15,23,42,1)_45%),linear-gradient(180deg,#0f172a,#111827)] px-4 py-6 text-white sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        <header className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-[0_24px_70px_rgba(2,6,23,0.38)] backdrop-blur">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Link to={`/${workspaceRole}/exercises/items`} className="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-semibold text-white">
              ...
              {mode === 'edit' && slug ? (
                <button
                  type="button"
                  onClick={() => {
                    if (window.confirm('Xoa bai tap nay?')) {
                ...
                >
                  {deleteMutation.isPending ? 'Dang xoa...' : 'Xoa bai tap'}
                </button>
              ) : null}
              <button
                type="button"
                ...
                className="rounded-full bg-[linear-gradient(135deg,#f59e0b,#b45309)] px-5 py-3 text-sm font-semibold text-white disabled:opacity-60"
              >
                {saveMutation.isPending ? 'Dang luu...' : mode === 'edit' ? 'Luu cap nhat' : 'Tao bai tap moi'}
              </button>
            </div>
          </div>
        ...
        {mode === 'edit' && detailQuery.isPending ? <QueryLoadingState title="Dang tai chi tiet bai tap..." /> : null}
        {courseOptionsQuery.isPending ? <QueryLoadingState title="Dang tai course options..." /> : null}
        {exerciseTopicsQuery.isPending ? <QueryLoadingState title="Dang tai chu de bai tap..." /> : null}
        {detailQuery.error ? <QueryErrorState title="Khong tai duoc chi tiet bai tap" description={getApiErrorMessage(detailQuery.error)} /> : null}
        ...
        {exerciseTopicsQuery.error ? <QueryErrorState title="Khong tai duoc chu de bai tap" description={getApiErrorMessage(exerciseTopicsQuery.error)} /> : null}
        {saveMutation.error ? <QueryErrorState title="Khong luu duoc bai tap" description={getApiErrorMessage(saveMutation.error)} /> : null}
        {deleteMutation.error ? <QueryErrorState title="Khong xoa duoc bai tap" description={getApiErrorMessage(deleteMutation.error)} /> : null}
          ...
          <aside className="rounded-[2rem] border border-white/10 bg-white/5 p-5 shadow-[0_18px_55px_rgba(2,6,23,0.3)] backdrop-blur">
            ...
            <div className="mt-5 grid gap-3">
              <input
                ...
                className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-slate-400 focus:border-amber-300"
              />
              <div className="grid grid-cols-2 gap-3">
                <select
                  value={draft.type}
                  onChange={(event) => setDraft((current) => ({ ...current, type: event.target.value as MockTestPayload['type'] }))}
                  className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none focus:border-amber-300"
                >
              ...
              <select
                value={draft.exerciseTopicSlug ?? ''}
                onChange={(event) => setDraft((current) => ({ ...current, exerciseTopicSlug: event.target.value }))}
                className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none focus:border-amber-300"
              >
                <option value="">Chon chu de</option>
                {availableTopics.map((topic: ExerciseTopic) => (
                  <option key={topic.id} value={topic.slug}>{topic.label}</option>
                ))}
              </select>
              ...
              <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-slate-200">
                <input
                  ...
                  className="size-4"
                />
                Featured tren khu bai tap cong khai
              </label>
            </div>
            ...
            <div className="mt-5 rounded-[1.4rem] border border-white/10 bg-slate-950/35 px-4 py-4">
              ...
              <p className="mt-3 text-xs leading-6 text-slate-400">
                Bai tap co the gan vao khoa hoc de tao track on tap rieng cho nhom hoc vien, hoac de rong de xuat hien cong khai trong /exercises.
              </p>
            </div>
            ...
            <div className="mt-5 rounded-[1.4rem] border border-white/10 bg-slate-950/35 px-4 py-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-amber-400 text-slate-950"
                >
                  +
                </button>
              </div>
              <div className="mt-4 grid grid-cols-5 gap-2">
                {draft.questions.map((question, index) => (
                  <button
                    key={`exercise-question-nav-${index}`}
                    type="button"
          ...
          <section className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-[0_18px_55px_rgba(2,6,23,0.3)] backdrop-blur">
            <div className="grid gap-5">
              <article className="rounded-[1.5rem] border border-white/10 bg-slate-950/35 px-5 py-5">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-200">Mo ta bai tap</p>
                <textarea
                  rows={4}
                  value={draft.description}
                  onChange={(event) => setDraft((current) => ({ ...current, description: event.target.value }))}
                  placeholder="Mo ta bai tap"
                  className="mt-4 w-full rounded-[1.5rem] border border-white/10 bg-white/5 px-4 py-3 text-sm leading-7 text-white outline-none placeholder:text-slate-400 focus:border-amber-300"
                />
              </article>

              <article className="rounded-[1.5rem] border border-white/10 bg-slate-950/35 px-5 py-5">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-200">Huong dan lam bai</p>
                <textarea
                  rows={4}
              ...
              {currentQuestion ? (
                <article className="rounded-[1.5rem] border border-white/10 bg-slate-950/35 px-5 py-5">
                  ...
                  <textarea
                    ...
                    placeholder="Explanation ho tro hoc vien sau khi nop bai"
                    className="mt-4 w-full rounded-[1.5rem] border border-white/10 bg-white/5 px-4 py-3 text-sm leading-7 text-white outline-none placeholder:text-slate-400 focus:border-amber-300"
                  />
                  ...
                  <div className="mt-4 grid gap-3">
                    {currentQuestion.options.map((option, optionIndex) => (
                      <label key={`${option.key}-${optionIndex}`} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                        <input
                          type="radio"
                          name={`exercise-correct-answer-${currentQuestionIndex}`}
                        ...
                        <input
                          ...
                          className="min-w-0 flex-1 rounded-xl border border-white/10 bg-slate-950/45 px-3 py-2 text-white outline-none placeholder:text-slate-400 focus:border-amber-300"
                        />
                      </label>
                    ))}
                  </div>
