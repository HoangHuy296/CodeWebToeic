...
export function MockTestCard({ mockTest }: { mockTest: MockTest }) {
  ...
  const isTeacherOwner =
    role === "teacher" && user?.id === mockTest.createdBy.id;
  ...
  const studentMockTestPath = isPublicExerciseFlow
    ? `/student/mock-tests/${mockTest.id}?from=public`
    : `/student/mock-tests/${mockTest.id}`;
  const isOwner =
    (role === "teacher" || role === "admin") &&
    user?.id === mockTest.createdBy.id;
  const isFreeTest =
    mockTest.status === "published" && mockTest.assignedCourseIds.length === 0;
